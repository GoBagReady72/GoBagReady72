...
/**
 * Ready72 Wireframes v0.4.3 — Tailwind-free, fully typed, high-contrast persona cards
 * Scenes: Intro (0), Persona (1), Briefing (2)
 */

// ---- Minimal CSS injection (no Tailwind required) ----
function useMinimalCSS(): void {
  useEffect(() => {
    ...
    style.textContent = `
      ...
      /* Card typography contrast hotfix */
      .r72-card-title { color: #FFFFFF; }      /* pure white */
      .r72-card-blurb { color: #CBD5E1; }      /* slate-300 */
      .r72-card-list li { color: #E5E7EB; }    /* slate-200 */
    `;
...
// ---- Scene 1 — Persona ----
type Persona = { key: PersonaKey; title: string; cash: number; blurb: string; stats: string[] };
...
type ScenePersonaProps = { onContinue: (persona: PersonaKey, cash: number) => void };
...
// ---- Scene 2 — Briefing ----
type SceneBriefingProps = { persona: PersonaKey; cash: number; onAdvance: ()=>void };
...
// ---- Demo container ----
export default function Ready72WireframesV043(): JSX.Element {
  const [scene, setScene] = useState<0|1|2>(0);
  const [persona, setPersona] = useState<PersonaKey>('EC');
