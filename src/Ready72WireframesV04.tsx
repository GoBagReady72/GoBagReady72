...
/**
 * Ready72 Wireframes v0.4 — Canon-aligned
 * - HazAssist palette (#020817 / #0B1220 / #F97316)
 * - Top Command HUD with rotating advisor prompt
 * - Bottom status HUD
 * - Personas: EC ($300), Prepper ($150)
 * - Region is hidden when not set
 * - Copy simplified for ages 10–50
 */
      ...
      <Card className="p-3 mb-3">
        <div className="flex flex-col gap-2">
          <div className="flex flex-wrap items-center gap-3 text-sm">
            ...
            {region ? (<div><span style={{ color: HA.muted }}>Region:</span> {region}</div>) : null}
    ...
    <Shell title="Scene 1 · Choose your profile">
        ...
        <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
          {PERSONA.map(p => (
            ...
            >
              ...
              <div className="mt-3 text-xs uppercase tracking-wide" style={{ color: HA.muted }}>What to expect</div>
      ...
      <div className="max-w-6xl mx-auto mt-6 flex items-center justify-between">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg font-semibold ${!picked ? 'cursor-not-allowed' : ''}`}
          style={{
            backgroundColor: picked ? HA.accent : HA.border,
            color: HA.text,
            opacity: picked ? 1 : 0.5,
            filter: picked ? 'none' : 'grayscale(40%)'
          }}
...
// Demo container
export default function Ready72WireframesV04() {
