...
/**
 * Ready72 Wireframes v0.7 — Adds Scene 5: Debrief (MSS Report + Outcome)
 * Flow: 0 Intro → 1 Persona → 2 Briefing → 3 Store → 4 Decision → 5 Debrief
 * - Tailwind-free CSS via injected styles
 * - Fully typed
 */
...
// ---- Scene stubs reused from v0.6 (Intro, Persona, Briefing, Store, Decision) ----
// For brevity, these scenes are shortened; functionality mirrors v0.6.
      ...
      <div className="r72-row">
        ...
        <Card style={{ flex: 2, padding: 16 }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            {PERSONA.map(p=>(
              <button key={p.key} onClick={()=>setPicked(p.key)} className="r72-card" style={{ padding:12, textAlign:'left', border:`1px solid ${picked===p.key?HA.accent:HA.border}` }}>
                <div style={{ fontWeight:600 }}>{p.title}</div>
                <div className="r72-muted" style={{ fontSize:13 }}>{p.blurb}</div>
              </button>
            ))}
          </div>
          <div style={{ display:'flex', justifyContent:'flex-end', marginTop:12 }}>
            <button disabled={!picked} className="r72-btn primary" onClick={()=>onContinue(selectedPersona, selectedPersona.cash)}>Continue to briefing</button>
          </div>
        </Card>
      </div>
    </Shell>
...
const SceneBriefing: React.FC<SceneBriefingProps> = ({ persona, cash, onAdvance }) => (
  <Shell title="Scene 2 · Emergency scenario briefing">
    ...
    <div className="r72-row">
      ...
      <Card style={{ flex: 2, padding: 16 }}>
        <div style={{ fontWeight:600, marginBottom: 8 }}>Severe Flood Watch — example region</div>
        <GradientBar />
        <div style={{ display:'flex', justifyContent:'flex-end', marginTop: 12 }}>
  ...
  return (
    <Shell title="Scene 3 · MSS Store / Loadout">
      ...
      <div className="r72-row">
        <div style={{ flex: 1 }}>
          <AdvisorChat lines={[
            "Balance first: Water, Food, Shelter, Health, Comms, Sustainability.",
            "Stay under cash and weight limits.",
          ]} />
        </div>
        <Card style={{ flex: 2, padding: 16 }}>
...
// ---- Decision Loop (simplified from v0.6) ----
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            ...
            <div>
              ...
              <div style={{ marginTop: 12, display:'flex', justifyContent:'flex-end', gap:8 }}>
                <button className="r72-btn" onClick={()=>{ /* end now */ ; (setHoursLeft as any)(0); }}>Force End</button>
              </div>
            </div>
          </div>
        </Card>
...
const SceneDebrief: React.FC<SceneDebriefProps> = ({ persona, cart, win, log, onRestart }) => {
  ...
  // FRS outcome heuristic: demo only
      ...
      <div className="r72-row">
        ...
        <div style={{ flex: 1 }}>
          ...
          <Card style={{ padding: 12, marginTop: 12 }}>
            ...
            <div className="r72-log">
              {log.slice(0,6).map((l,i)=>(<div key={i} style={{ background:'#020817', border:`1px solid ${HA.border}`, borderRadius:10, padding:'8px 10px', fontSize:14 }}>{l}</div>))}
            </div>
          </Card>
        </div>
      </div>
...
// ---- Demo container with Scenes 0–5 ----
export default function Ready72WireframesV070(): JSX.Element {
